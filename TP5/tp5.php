...
<?php
function Q1(){fopen("config.ini", "r");
...
function Q2(){fopen("config.ini", "r");
    $HTMLscript = "<!DOCTYPE html>
    <html lang='en'>
    <head>
        <meta charset='UTF-8'>
    ...
    fclose($data);
    //créer un fichier html
    $nomFichier = $tabNouvellesValeurs[0].'.php';
    //mettre un form dans le contenu du fic php
    $HTMLscript .= "<form method='post'>";
    //parcourir le tableau
    for ($i=2; $i < count($tabNouvellesValeurs) ; $i++) { 
        //ajouter un label et un input
    ...
    //fermer les balises à la fin du fichier php
    $HTMLscript .= "</form> </body> </html>";
    //ouvrir le fichier en écriture
    $monFic = fopen($nomFichier,"w");
    //écrire le contenu
